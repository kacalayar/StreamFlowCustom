...
const manualStopOptions = new Map();
  ...
  try {
    ...
    ffmpegProcess.on('exit', async (code, signal) => {
      ...
      if (isManualStop) {
        ...
        const stopOptions = manualStopOptions.get(streamId);
        manuallyStoppingStreams.delete(streamId);
        manualStopOptions.delete(streamId);
        if (stopOptions && stopOptions.processed) {
          return;
        }
        if (wasActive) {
          try {
            if (!stopOptions || !stopOptions.keepStatusLive) {
              await Stream.updateStatus(streamId, 'offline');
            }
            if (typeof schedulerService !== 'undefined' && schedulerService.cancelStreamTermination && (!stopOptions || !stopOptions.skipSchedulerCallback)) {
...
async function stopStream(streamId, options = {}) {
  try {
    const {
      keepStatusLive = false,
      skipHistory = false,
      skipSchedulerCallback = false
    } = options;
    ...
    if (typeof schedulerService !== 'undefined' && schedulerService.cancelStreamTermination && !skipSchedulerCallback) {
      schedulerService.handleStreamStopped(streamId);
    }
    stopOptions.processed = true;
...
async function switchPlaylist(streamId, playlistId) {
  try {
    const stream = await Stream.findById(streamId);
    if (!stream) {
    ...
    addStreamLog(streamId, `Switching to playlist ${playlistId}`);

    if (activeStreams.has(streamId)) {
      const stopResult = await stopStream(streamId, {
        keepStatusLive: true,
        skipHistory: true,
        skipSchedulerCallback: true
      });
      if (!stopResult.success) {
        throw new Error(stopResult.error || 'Failed to stop stream before switching playlist');
      }
    }

    await Stream.update(streamId, { video_id: playlistId });
    const restartResult = await startStream(streamId);
    if (!restartResult.success) {
      throw new Error(restartResult.error || 'Failed to restart stream after switching playlist');
    }
    return { success: true };
  } catch (error) {
    console.error(`[StreamingService] Failed to switch playlist for stream ${streamId}:`, error);
    addStreamLog(streamId, `Failed to switch playlist: ${error.message}`);
    return { success: false, error: error.message };
  }
}
