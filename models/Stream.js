...
class Stream {
    ...
    return new Promise((resolve, reject) => {
      db.run(
        `INSERT INTO streams (
          ...
          schedule_time, duration, status, status_updated_at, use_advanced_settings, auto_playlist_mode, user_id
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          id, title, video_id, rtmp_url, stream_key, platform, platform_icon,
          bitrate, resolution, fps, orientation, loop_video_int,
          schedule_time, duration, status, status_updated_at, use_advanced_settings_int, auto_playlist_mode_int, user_id
        ],
        function (err) {
          if (err) {
    ...
    return new Promise((resolve, reject) => {
      db.get('SELECT * FROM streams WHERE id = ?', [id], (err, row) => {
        ...
        if (row) {
          ...
          row.auto_playlist_mode = row.auto_playlist_mode === 1;
        }
        resolve(row);
      });
    });
  ...
  static update(id, streamData) {
    ...
    Object.entries(streamData).forEach(([key, value]) => {
      if ((key === 'loop_video' || key === 'auto_playlist_mode') && typeof value === 'boolean') {
        fields.push(`${key} = ?`);
