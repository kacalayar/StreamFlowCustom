...
function createTables() {
  ...
  db.run(`CREATE TABLE IF NOT EXISTS streams (
    ...
    auto_playlist_mode BOOLEAN DEFAULT 0,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    user_id TEXT,
    FOREIGN KEY (user_id) REFERENCES users(id),
  ...
  db.run(`CREATE TABLE IF NOT EXISTS playlist_schedules (
    id TEXT PRIMARY KEY,
    ...
    if (err) {
      console.error('Error creating playlist_schedules table:', err.message);
    }
  });

  db.run(`CREATE INDEX IF NOT EXISTS idx_playlist_schedules_stream ON playlist_schedules(stream_id)`);
  db.run(`CREATE INDEX IF NOT EXISTS idx_playlist_schedules_day ON playlist_schedules(day_of_week, start_minutes)`);
  ...
  db.run(`ALTER TABLE streams ADD COLUMN auto_playlist_mode BOOLEAN DEFAULT 0`, (err) => {
    if (err && !err.message.includes('duplicate column name')) {
      console.error('Error adding auto_playlist_mode column:', err.message);
    }
  });
}
